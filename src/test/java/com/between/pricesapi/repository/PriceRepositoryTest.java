...
import com.between.pricesapi.service.PriceServiceImpl;
...
public class PriceRepositoryTest {
    ...
    public void setup(){
        ...
        Price price = Price.builder().id(1L).productId(productId).
                ...
                brandId(brandId).
                build();
        Mockito.when(priceRepository.getPriceByDateAndBrandAndProduct(applicationDate, productId, brandId)).thenReturn(price);
    }
    ...
    void when_all_arguments_are_ok_then_return_price_ok(){
        ...
        Price price = priceService.getPriceByDateAndBrandAndProduct(applicationDate, productId, brandId);
    ...
    void when_some_arguments_are_empty_date_then_return_price_null(){
        ...
        Price price = priceService.getPriceByDateAndBrandAndProduct(null, productId, brandId);
        Assertions.assertThat(price).isNull();

        price = priceService.getPriceByDateAndBrandAndProduct(applicationDate, null, brandId);
        Assertions.assertThat(price).isNull();

        price = priceService.getPriceByDateAndBrandAndProduct(applicationDate, productId, null);
        Assertions.assertThat(price).isNull();
    }
}
